...
import { Destroy } from '../../data/destroy';
...
export class CountUpDirective implements OnInit {
  ...
  checkScroll() {
    let element = document.getElementById('outstanding');
    let widthWindow = window.innerWidth >= 1920 ? 500 : window.innerWidth >= 1023 ? 400 : window.innerWidth >= 1023 ? 300 : 300;
    if (element) {
        let elementOffsetTop = element.offsetTop;
        if (((document.documentElement.scrollTop + widthWindow) >= elementOffsetTop) && this.loading === false) {
            this.loading = true
            return this.displayCurrentCount();
